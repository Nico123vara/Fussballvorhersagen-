    ...
    try:
        query = """
            SELECT
                ...
                p.prob_draw,
                p.prob_away,
                p.predicted = m.winner AS korrekt
            FROM predictions p
            JOIN matches m  ON p.match_id = m.id
            JOIN teams t1   ON m.home_team_id = t1.id
            JOIN teams t2   ON m.away_team_id = t2.id
...
def show_prob_bars(prob_dict):
    ...
    for label, prob in sorted(prob_dict.items(), key=lambda x: -x[1]):
        ...
        st.markdown(
            ...
            f'<span style="color:{"#000000" if ist_beste else "#555555"};'
...
def show_team_stats(matches, team_id, team_name, season_id):
    ...
    with col1:
        st.metric("Punkte letzte 5 Spiele", f"{get_form(matches, team_id, season_id)}/15")
    with col2:
        st.metric("Heimquote", f"{get_heimquote(matches, team_id, season_id)*100:.0f}%")
    with col3:
        st.metric("Siegesserie", get_win_streak(matches, team_id, season_id))
    with col4:
        gd = get_goal_diff(matches, team_id, season_id)
        st.metric("Ø Tordifferenz letzte 5 Spiele", f"{gd:+.1f}")

    st.caption("Punkte letzte 5 Spiele — Sieg=3, Unentschieden=1, Niederlage=0. Maximum 15 Punkte.")
    st.caption("Heimquote — Anteil der gewonnenen Heimspiele aus den letzten 12 Heimspielen.")
    st.caption("Siegesserie — Anzahl aufeinanderfolgender Siege bis zum letzten Spiel.")
    st.caption("Ø Tordifferenz letzte 5 Spiele — Durchschnittliche Tordifferenz (Tore geschossen minus kassiert). Positiv = mehr geschossen als kassiert.")
    ...
    for _, s in spiele.iterrows():
        ...
        st.markdown(
            ...
            f'<span style="color:#888888;font-size:0.8rem;width:80px">'
            f'{s["utc_date"].strftime("%d.%m.%Y")}</span>'
            f'<span style="background:#f0f0f0;padding:2px 6px;border-radius:4px;'
            f'font-size:0.75rem;color:#555555">{ort}</span>'
...
def main():
    ...
    <div style="display:flex;align-items:center;gap:16px;margin-bottom:1rem">
        <img src="https://crests.football-data.org/BL1.png" width="50">
        <span style="font-size:2rem;font-weight:800;color:#111111">Bundesliga Vorhersagen</span>
    </div>
    """, unsafe_allow_html=True)
    st.markdown("---")
    ...
    # ── Tab 1: Vorhersage + Teamstatistiken ──
    with tab1:
        ...
        with c1:
            st.metric("Punkte letzte 5 Spiele", f"{home_feat['form']}/15")
            st.metric("Heimquote", f"{home_feat['heimquote']*100:.0f}%")
            st.metric("Ø Tordifferenz letzte 5 Spiele", f"{home_feat['goal_diff']:+.1f}")
        with c2:
            st.markdown("<div style='text-align:center;color:#888888;padding-top:24px'>vs</div>",
                       unsafe_allow_html=True)
        with c3:
            st.metric("Punkte letzte 5 Spiele", f"{away_feat['form']}/15")
            st.metric("Auswärtsform letzte 5 Spiele", f"{away_feat['form_away']}/15")
            st.metric("Ø Tordifferenz letzte 5 Spiele", f"{away_feat['goal_diff']:+.1f}")

        st.caption("Punkte letzte 5 Spiele — Sieg=3, Unentschieden=1, Niederlage=0. Maximum 15 Punkte.")
        st.caption("Heimquote — Anteil der gewonnenen Heimspiele des Heimteams aus den letzten 12 Heimspielen.")
        st.caption("Auswärtsform letzte 5 Spiele — Punkte des Auswärtsteams aus den letzten 5 Auswärtsspielen.")
        st.caption("Ø Tordifferenz letzte 5 Spiele — Durchschnittliche Tordifferenz. Positiv = mehr geschossen als kassiert.")

        # Teamstatistiken als Untertabs
        st.markdown("---")
    ...
    # ── Tab 2: Nächste 4 Wochen ──
    with tab2:
        st.markdown("### Spiele der nächsten 4 Wochen")
        upcoming = load_upcoming_matches()

        if not upcoming:
            st.info("Keine Spiele in den nächsten 4 Wochen — wahrscheinlich Sommerpause. 🌴")
            ...
            for m in upcoming:
                ...
                with col_pred:
                    if not home_row.empty and not away_row.empty:
                        ...
                        st.markdown(
                            f'<span style="background:{farbe};color:#ffffff;padding:4px 12px;'
    ...
    # ── Tab 3: Genauigkeit ──
    with tab3:
        st.markdown("### 🎯 Meine Vorhersage-Genauigkeit")

        stats = load_prediction_stats()

        if stats is None or stats['total'] == 0:
            st.info("Noch keine abgeschlossenen Spiele mit Vorhersagen.")
            st.caption("Führe save_predictions.py aus um Vorhersagen zu speichern.")
        else:
            ...
            with col1:
                st.metric("Korrekte Vorhersagen", f"{korrekt}/{total}")
            with col2:
                st.metric("Genauigkeit", f"{genauigkeit:.1f}%")
            with col3:
                st.metric("Baseline (immer Heimsieg)", "~43%")
